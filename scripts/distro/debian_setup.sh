...
update_system() {
    print_header "Updating Debian System Packages"
    if ! command -v apt >/dev/null 2>&1; then
        die "This script requires apt (Debian)."
    fi
    sudo apt update -y && sudo apt full-upgrade -y && sudo apt autoremove -y && sudo apt autoclean -y
}

install_core_tools() {
    print_header "Installing Core Packages & Tools for Debian"
...
purge_old_editors() {
    ...
    sudo apt purge -y vim vim-runtime vim-common || true
    sudo apt purge -y neovim || true
    sudo apt autoremove -y
}

install_ssh_server() {
    print_header "Installing SSH Server for Debian"
    if confirm "Do you want to install SSH server?"; then
        ...
        printf "%bChecking for SSH package conflicts...%b\n" "${C_YELLOW}" "${C_DEFAULT}"
        
        # Try installing directly first
        if sudo apt install -y openssh-server; then
            printf "%bSSH server installed successfully%b\n" "${C_GREEN}" "${C_DEFAULT}"
        else
            printf "%b%sSSH server installation failed. Attempting conflict resolution...%b\n" "${C_RED}${C_BOLD}" "" "${C_DEFAULT}"
            
            # Try to resolve conflicts by reinstalling openssh-client
            if confirm "Try to resolve SSH package conflicts automatically?"; then
                sudo apt remove --purge -y openssh-client openssh-server || true
                sudo apt autoremove -y
                
                if sudo apt install -y openssh-server openssh-client; then
                    printf "%bSSH packages installed successfully after conflict resolution%b\n" "${C_GREEN}" "${C_DEFAULT}"
                else
                    printf "%bAutomatic conflict resolution failed. Manual intervention required.%b\n" "${C_RED}" "${C_DEFAULT}"
                    printf "%bPlease check your package sources and try installing SSH manually.%b\n" "${C_YELLOW}" "${C_DEFAULT}"
                    return 1
                fi
            else
                return 1
...
main_menu() {
    ...
    # Check for Debian or Debian-based systems
    if ! grep -qiE "debian|ID_LIKE=.*debian" /etc/os-release; then
    ...
    printf "%b%s%b\n" "${C_BLUE}${C_BOLD}" "Welcome to the Debian Development Environment Setup Script" "${C_DEFAULT}"
    printf "%bDetected OS: %b%s\n" "${C_GREEN}" "${C_DEFAULT}" "$(lsb_release -d 2>/dev/null | cut -f2 || echo 'Debian')"
